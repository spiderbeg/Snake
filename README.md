...
2. 创建主要角色 蛇、食物，背景
   ...
   2. 食物
   3. 背景
3. 最终代码完成(**mySnake.py**，在代码 140 行为蛇移动的时间间隔，数值越小速度越快；代码 168 行为画线条，形成格子。注释掉此行则不显示格子)。
4. 更多实用而有趣的分析案例请关注：<strong>Crossin的编程教室</strong>
## 项目思路
* 考虑一下，在我们要自己写贪吃蛇这个游戏时。我们应该想到，在这个游戏中有蛇、食物、背景三个对象，当然你可以认为主要是蛇和食物两个对象也没关系。这里以三个对象来描述游戏。那么我们就来分析一下，各个对象都有哪些方法。
* 蛇
